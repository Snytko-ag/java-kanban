...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Task task1 = new Task(manager.getNewId(),"задача1", "собрать коробки",  StatusTask.NEW);
        Task task2 = new Task(manager.getNewId(),"задача2", "собрать книги",  StatusTask.NEW);

        Epic epic1 = new Epic(manager.getNewId(),"epic1", "перезд",  StatusTask.NEW);
        Epic epic2 = new Epic(manager.getNewId(),"epic2", "регистрация",  StatusTask.NEW);

        Subtask subtask1 = new Subtask(manager.getNewId(),"подзадача1", "собрать вещи",  StatusTask.NEW, epic1.getIdTask());
        Subtask subtask2 = new Subtask(manager.getNewId(),"подзадача2", "погрузить вещи",  StatusTask.NEW, epic1.getIdTask());
        Subtask subtask3 = new Subtask(manager.getNewId(),"подзадача3", "найти документы",  StatusTask.NEW, epic2.getIdTask());
        Subtask subtask4 = new Subtask(manager.getNewId(),"подзадача4", "сходить в паспортный стол",  StatusTask.NEW, epic2.getIdTask());
        ...
        Subtask subtask6 = new Subtask(subtask4.getIdTask(), "подзадача4", "сходить в паспортный стол",  StatusTask.DONE, epic2.getIdTask());
        ...
        System.out.println(manager.searchEpicById(subtask4.getEpicId()));
